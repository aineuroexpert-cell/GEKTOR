...
$remote_cmd = "cd " + $TARGET_DIR + " && tar -xzf " + $ARCHIVE + " && rm " + $ARCHIVE + " && source venv/bin/activate && pip install -r requirements.txt && systemctl daemon-reload && systemctl start gektor.service && systemctl status gektor.service --no-pager"
