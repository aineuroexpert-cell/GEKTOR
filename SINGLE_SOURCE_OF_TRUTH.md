...
5. **Изоляция символов запрещена.** Любой символ анализируется с обязательной привязкой к BTC/ETH macro-контексту (когда такая привязка будет введена в этой ветке развития — на v3.6.2 этого нет, но запрет действует на будущие изменения).
...
# Базлайн v3.6.2 (радар-контур): 83 passed, 39 warnings.
#   • Было 54 в v3.6.1.
#   • +29 новых: 16 liquidity_detectors + 7 adaptive_threshold + 6 sensitivity_tiers.
```
...
Эти контуры существуют в репо для других режимов (Trading Mode, RAG-помощник, L2 микроструктура). В v3.6.2 APEX-RADAR они не используются и не блокируют производственную работу радара.
